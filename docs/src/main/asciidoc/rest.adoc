...
This feature is introduced as a Technology Preview and is enabled by default.
...
To turn this feature off, set the <<quarkus-rest-jackson_quarkus-rest-jackson-optimization-enable-reflection-free-serializers,`quarkus.rest.jackson.optimization.enable-reflection-free-serializers`>> configuration property to `false`:
...
When enabled, {project-name} generates, at build time, a property accessor that reads and writes the properties of the classes requiring JSON conversion through direct calls instead of reflection. Jackson keeps building and driving its own serializers and deserializers, and only their reflective property access is replaced, so every Jackson feature keeps working; a property the accessor does not cover simply falls back to reflection.
