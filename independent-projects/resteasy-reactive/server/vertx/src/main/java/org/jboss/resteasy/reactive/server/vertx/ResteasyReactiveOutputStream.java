...
public class ResteasyReactiveOutputStream extends OutputStream {
    ...
    private final AppendBuffer writeBatch;
    ...
    public void write(final byte[] b, final int off, final int len) throws IOException {
        ...
        try {
            while (rem > 0) {
                final int written = writeBatch.append(b, idx, rem);
                if (written < rem) {
    ...
    public void flush() throws IOException {
        ...
        try {
            var toFlush = writeBatch.flushBatch();
            if (toFlush != null) {
    ...
    public void close() throws IOException {
        if (closed)
            return;
        try {
            writeBlocking(writeBatch.flushBatch(), true);
        } catch (Exception e) {
            throw new IOException(e);
        } finally {
            closed = true;
